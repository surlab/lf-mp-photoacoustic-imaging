...
% source parameters
source_f0       = 1e+6;      % source frequency [Hz]
source_diam     = 75e-6;    % piston diameter [m]
source_amp      = 1e+4;      % source pressure [Pa]
source_lateral = 5e-6;
source_axial = 25e-6;

% grid parameters
axial_size      = 1e-3;    % total grid size in the axial dimension [m]
lateral_size    = 1e-3;    % total grid size in the lateral dimension [m]
...
karray.addRectElement([kgrid.x_vec(1), 0, 0],source_axial,source_lateral,[0,0,0]);
...
source.p0 = makeBall(Nx,Ny,Nz,0,0,0,radius);
%source.p '=' source.p0(Nx/4, Ny/2 - source_radius:Ny/2 + source_radius, Nz/2 - source_radius:Nz/2 + source_radius) = 1;
%}
% --------------------
% MEDIUM
% --------------------
...
%figure;
%plot( 1e3*x_ref, 1*p_ref, 'k-');
%hold on;
plot( 1e3*x_vec, 1*amp_on_axis, 'b.');
hold off;
%set(gca, 'XLim', [0, axial_size] * 1e3, 'YLim', [0, 1.1] * source_amp * 2e-6);
xlabel('Axial Position [mm]');
ylabel('Pressure [10^3 Pa]');
...
imagesc(1e3 * y_vec, 1e3 * x_vec, C1);  % amp is the 2 D signal
colorbar;
colormap hot;
xlabel('Lateral Position [mm]');
ylabel('Axial Position [mm]');
axis image;
title('Pressure Field in arbitrary pressure units');
